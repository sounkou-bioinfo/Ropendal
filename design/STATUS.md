...
| `store_cache()` explicit local byte-store cache | yes | yes | yes | no | full-object cache for complete store-key reads using a local OpenDAL `fs` cache store; tests cover validation, stale-cache mode, byte handles, vectorized and async reads, bypassed ranges, invalidation, async mutation wrappers, clear, and pure-R chunked-array cache refresh |
...
| native store cache adapters | yes | yes | no | yes | `ropendal_store_cache_open()` wraps uncached parent/cache stores as an explicit full-object cache; `ropendal_store_block_cache_open()` adds a fixed-size range-aware block cache; roundtrip covers fill, last-modified/size validation, read-into/range assembly, and cached-store mutation invalidation |
