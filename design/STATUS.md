...
| `byte_store()` / `store_*()` key-to-bytes adapter | yes | yes | yes | no | R-side prefix adapter for Zarr-like layouts; `store_read()` is bytes-first; local tests cover raw and byte-handle reads, vectorized writes, create-vs-replace, listing path rewriting, async wrappers, delete, and escape rejection |
| `store_cache()` explicit local byte-store cache | yes | yes | yes | no | full-object cache for complete store-key reads using a local OpenDAL `fs` cache store; tests cover validation, stale-cache mode, byte handles, vectorized and async reads, bypassed ranges, invalidation, async mutation wrappers, and clear |
...
| native store cache adapter | yes | yes | no | yes | `ropendal_store_cache_open()` wraps uncached parent/cache stores as an explicit full-object cache; roundtrip covers fill, last-modified/size validation, and cached-store mutation invalidation |
...
2. Extend the `OpendalBytes` byte boundary with an ALTREP raw facade if profiling warrants it; the safe design is `data1` holding the external byte holder, `Elt`/`Get_region` copying regions, read-only `Dataptr()` only for stable contiguous storage, and writable `Dataptr()` materializing an R-owned `RAWSXP`. C API byte-handle, byte-store, and store-cache accessors are implemented; `byte_ranges()` request objects cover index-heavy R range readers; `byte_store()` provides a bytes-first prefix-based key-to-bytes substrate for Zarr-like layouts; and `store_cache()` adds an explicit local full-object cache for complete chunk-key reads.
