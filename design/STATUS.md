...
| `byte_store()` / `store_*()` key-to-bytes adapter | yes | yes | yes | no | R-side prefix adapter for Zarr-like layouts; local tests cover raw and byte-handle reads, vectorized writes, create-vs-replace, listing path rewriting, delete, and escape rejection |
| `store_cache()` explicit local byte-store cache | yes | yes | yes | no | full-object cache for complete store-key reads using a local OpenDAL `fs` cache store; tests cover validation, stale-cache mode, byte handles, vectorized reads, bypassed ranges, invalidation, and clear |
