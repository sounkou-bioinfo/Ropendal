...
The async API is inspired by `nanonext`-style Aio handles: issue async work with `fs_*_aio()`, then `call_aio()` to wait for completion and update the Aio object, and `collect_aio()` to retrieve the result (or an `opendalErrorValue`).
