...
expect_equal(store_read(store, "zarr.json"), charToRaw("{}"))
...
expect_equal(
  store_read(store, c("c/0/0", "c/0/1"), batch_concurrency = 2),
  list(as.raw(c(1, 2, 3, 4)), as.raw(c(5, 6, 7, 8)))
)
expect_equal(store_read(store, "c/0/0", offset = 1, size = 2), as.raw(c(2, 3)))
chunk_bytes <- store_read(store, "c/0/1", mode = "bytes")
...
expect_equal(store_read(store, "sub/../zarr.json"), charToRaw("[]"))

root_store <- byte_store(fs)
expect_true(identical(store_write(root_store, "root-key.bin", as.raw(11)), TRUE))
expect_equal(store_read(root_store, "root-key.bin"), as.raw(11))
